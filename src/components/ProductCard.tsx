...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="group cursor-pointer block">
      {/* Image Container */}
      <div
        className="relative aspect-[4/5] bg-muted mb-6 overflow-hidden"
        onMouseEnter={() => {
          if (images.length > 1) {
            setHoverIndex(1);
          }
      ...
      >
        ...
        {/* Discount Badge */}
        {discount > 0 && (
          <div className="absolute top-4 right-4 bg-destructive text-destructive-foreground px-3 py-1 text-xs font-bold z-10">
            -{discount}%
          </div>
        )}
